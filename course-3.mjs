/**
 * This is an example referring to https://github.com/WTFAcademy/WTF-Ethers/tree/main/03_ReadContract
 * with the following changes:
 * 1. An extra showcase on how to fetch WTF Academy contract information
 * 2. Changed to Infura endpoint for JSON RPC provider.
 */
...
const displayWTFContractInfo = async () => {
    ...
    const wtfContract = new ethers.Contract(contractAddress, abi, new ethers.providers.JsonRpcProvider(INFURA_GOERLI_URL));
    const name = await wtfContract.name()
    const symbol = await wtfContract.symbol()
    const veryBigWoodsBalance = await wtfContract.balanceOf('0x0f46E7FF0Df5CFcE65d3aACf5a26268787aa8b79', 0)
    console.log("3. 读取WTF合约信息")
    console.log(`合约地址: ${contractAddress}`)
    console.log(`名称: ${name}`)
    console.log(`代号: ${symbol}`)
    const passed = veryBigWoodsBalance > 0 ? 'yes' : 'no'
    console.log(`VeryBigWoods Passed: ${passed}`)
}
