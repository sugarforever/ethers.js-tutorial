...
import * as contractJson from "./ecdsa-signature.json" assert {type: "json"};
...
const INFURA_SEPOLIA_URL = `https://sepolia.infura.io/v3/${process.env.INFURA_API_KEY}`
...
const main = async () => {
    ...
    if(ethers.utils.formatEther(balanceETH) > 0.002){
        ...
        let tx = await contractNFT.mint(account, "1", signature)
